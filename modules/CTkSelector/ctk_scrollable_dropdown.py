...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    ...
    def __init__(self, attach, x=None, y=None, button_color=None, height: int = 200, width: int = None,
                 ...
                 hover_color=None, arrow_color=None, font=None, text_noMatch='', text_noMatch_color=None, **button_kwargs):
        ...
        self.text_noMatch = customtkinter.ThemeManager.theme["CTkLabel"]["text"] if text_noMatch is None else text_noMatch
        self.text_noMatch_color = customtkinter.ThemeManager.theme["CTkLabel"]["text_color"] if text_noMatch_color is None else text_noMatch_color
        self.arrow_color = customtkinter.ThemeManager.theme["CTkLabel"]["text_color"] if arrow_color is None else arrow_color
        self.hover_color = customtkinter.ThemeManager.theme["CTkButton"]["hover_color"] if hover_color is None else hover_color
        
        self._font = customtkinter.ThemeManager.theme["CTkButton"]["font"] if font is None else font 
        ...
        self.no_match = customtkinter.CTkLabel(self.frame, text=self.text_noMatch, font=self._font, text_color=self.text_noMatch_color)
        self.height = height
        ...
        self.botonEspecial = []
        self.temp = []
        self.vistaMostrar = None
        self.vistaOcultar = None
        self.listaWidgetsEntry = []
        ...
        self.attach._canvas.itemconfig("dropdown_arrow",
                                        fill=self._apply_appearance_mode(self.arrow_color))
    ...
    def _init_buttons(self, **button_kwargs):
        ...
        for row in self.values:
            ...
            if len(self.botonEspecial) > 0:
                self.widgets[self.i] = customtkinter.CTkButton(self.frame,
                                                                text=self.botonEspecial[0][0],
                                                                height=self.button_height,
                                                                fg_color='transparent',
                                                                text_color=self.botonEspecial[0][1],
                                                                hover_color=self.hover_color,
                                                                anchor=self.justify,
                                                                font=self.botonEspecial[0][2],
                                                                command=lambda mostrar=self.vistaMostrar, ocultar=self.vistaOcultar: self.testeo(mostrar, ocultar, self.listaWidgetsEntry))
                self.widgets[self.i].pack(fill="x", pady=2, padx=(self.padding, 0))
    ...
    def live_update(self, string=None):
        ...
        if string:
            ...
            if i==1:
                if len(self.botonEspecial) > 0:
                    self.botonESP = customtkinter.CTkButton(self.frame,
                                                            text=self.botonEspecial[0][0],
                                                            height=self.button_height,
                                                            fg_color='transparent',
                                                            text_color=self.botonEspecial[0][1],
                ...
                if len(self.botonEspecial) > 0:
                    if len(self.temp) == 0:
                        self.temp.append(self.botonESP)
                        self.temp[0].pack(fill="x", pady=2, padx=(self.padding, 0))
                    elif len(self.temp) == 1:
                        # self.temp[0].pack_forget()
                        self.temp[0].pack(fill="x", pady=2, padx=(self.padding, 0))
                    elif len(self.temp) > 1:
                        self.temp.clear()
    ...
    def testeo(self, vistaMostrar, ocultar, widgetsEntry=[]):
        self.withdraw()
        mostrarVistas(vistaMostrar, ocultar, widgetsEntry)
        
        
    def cambiarVista(self, texto, colorTexto, fuente, vistaMostrar, vistaOcultar, widgetsEntry=[]):
        self.vistaMostrar = vistaMostrar
        self.vistaOcultar = vistaOcultar
        self.listaWidgetsEntry = widgetsEntry
        self.widgets[self.i] = customtkinter.CTkButton(self.frame,
                                                        text=texto,
                                                        height=self.button_height,
                                                        fg_color='transparent',
                                                        ...
                                                        command=lambda mostrar=self.vistaMostrar, ocultar=self.vistaOcultar: self.testeo(mostrar, ocultar, self.listaWidgetsEntry))
        self.widgets[self.i].pack(fill="x", pady=2, padx=(self.padding, 0))
        self.botonEspecial.append([texto, colorTexto, fuente])
        self.i+=1
